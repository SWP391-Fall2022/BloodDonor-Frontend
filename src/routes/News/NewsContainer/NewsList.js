...
export default class NewsList extends Component {
  ...
  render() {
    const checkedNews = Object.entries(this.props.categories)
      ...
      .map((category) => {
        if (category[0] === "campaignAct")
          return 1
        if (category[0] === "donorStory")
          return 2
        if (category[0] === "patientStory")
          return 3
        if (category[0] === "bloodType")
          return 4
        if (category[0] === "abo")
          return 5
    ...
    const search = (data) => {
      return data.filter((item) => (
        ...
        removeVietnameseTones(item.title)
          .toLowerCase()
          .includes(removeVietnameseTones(this.state.query.toLowerCase()))
